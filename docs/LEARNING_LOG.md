...
## STUDY-013 — 같은 시즌 해상도와 시간 전이 가능성

### 공부하게 된 이유

EXP-075의 park 효과는 같은 fold 정답을 허용하면 2023·2024 Skill 1000을 넘었지만 prior-only rolling에서는 크게 하락했다. 신호 존재와 배포 가능성을 분리할 필요가 있었다.

### 공부한 내용

- 현재 시즌의 home-team/park key는 target 평균 차이를 설명하지만 인접 시즌 효과 상관은 거의 0이거나 음수였다. 같은 key가 유지된다는 사실만으로 target 관계가 유지되지는 않는다.
- same-fold oracle은 “이 key에 해상도가 있는가”를 보여 주지만 “다음 시즌에 예측 가능한가”는 rolling-origin으로만 증명할 수 있다.
- 과거 두 시즌 부호 일치 필터도 2023·2024를 함께 개선하지 못했다. 과거 안정성 조건을 만족한 작은 집합도 unseen season의 구조 변화에 안전하지 않을 수 있다.
- career-stage처럼 합리적인 선수 메타 피처도 same-fold 상한 자체가 낮으면 파라미터 확장보다 조기 중단이 낫다.

### 프로젝트에 적용한 방법

- 홈 팀은 현재 행의 초/말과 양 팀 ID만으로 복원해 테스트 행 독립성을 유지했다.
- source residual은 시즌별 평균을 제거하고 각 source season을 별도 map으로 보존했다.
- family ceiling과 strict transfer를 함께 기록하고, ceiling은 높지만 transfer가 실패한 park 계열을 제출 후보에서 제외했다.
