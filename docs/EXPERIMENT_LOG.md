...
## EXP-072~073 — 동적 투수·타자 잠재 상태 AR(1)

### 실험 목적과 가설

기존 temporal posterior는 직전 시즌까지의 경력 누적 상태를 하나의 고정 prior로 사용했고, low-rank EB는 과거 source season 효과를 동일 평균했다. EXP-072는 최근 선수 상태가 시즌 사이에서 일부 지속되되 시간이 지나면 league 평균으로 감쇠한다는 가설을 검증했다. EXP-073은 같은 구조를 타자에게도 독립적으로 적합한 뒤 투수 `0.7`, 타자 `0.3`으로 결합했다.

### 기준 실험과 달라진 점

- 기준 예측은 Public 최고인 EXP-051 OOF `trackman_direct_recent_w010`으로 고정했다.
- 선수·시즌 성공률을 smoothing `200`으로 축소한 league-centered log-odds 상태로 변환했다.
- validation season보다 앞선 연속 시즌 선수 pair만 사용해 zero-intercept AR(1) `rho`를 ridge `1.0`, 범위 `[0,1]`로 적합했다.
- 최근 관측 시즌과 현재 시즌의 간격만큼 `rho ** gap`으로 상태를 감쇠했다.
- 현재 행의 공식 career `asof_n × rate`에서 학습 데이터의 직전 시즌 종료 누적값을 빼 현재 시즌 성공 수와 표본 수를 복원했다. 평가 데이터의 다른 행은 사용하지 않았다.
- 현재 fold 정답을 사용한 최적 1차원 가중치는 배포 불가 상한 진단으로만 저장했다.

### 모델과 주요 파라미터

| 실험 | 후보 | 주요 설정 |
| --- | --- | --- |
| EXP-072 | `ar_k30_w025`, `ar_k30_w050`, `ar_k100_w025`, `last_k30_w025` | state smoothing `200`, AR ridge `1`, current posterior strength `30/100`, correction weight `.25/.50` |
| EXP-073 | `joint_ar30_w025`, `joint_ar30_w050`, `batter_ar30_w025` | 투수·타자 별도 AR, joint `0.7/0.3`, posterior strength `30`, weight `.25/.50` |

EXP-072의 투수 `rho`는 2021~2024 prediction fold에서 `0.3573`, `0.3771`, `0.4497`, `0.3632`였다. EXP-073의 타자 `rho`는 `0.1670`, `0.2415`, `0.4087`, `0.2566`이었다.

### 검증 기간과 결과

모델·후보 선택에는 현재 fold를 쓰지 않았고, 2021 warm-up OOF부터 과거 fold worst Skill과 평균 Skill 순으로 다음 fold 후보를 정했다. 보고 기간은 2022·2023·2024다.

| 실험 | strict 선택 경로 | 2022 Brier / Skill | 2023 Brier / Skill | 2024 Brier / Skill | 평균 / 최저 Skill |
| --- | --- | --- | --- | --- | ---: |
| EXP-051 기준 | 고정 | 0.244862459 / 1726.23 | 0.247674466 / 930.21 | 0.247608043 / 880.23 | 1178.89 / 880.23 |
| EXP-072 | `last .25 → last .25 → AR .25` | 0.244843051 / 1734.02 | 0.247679853 / 928.06 | 0.247604267 / 881.74 | 1181.28 / 881.74 |
| EXP-073 | `joint .50 → joint .50 → joint .25` | 0.244848042 / 1732.02 | 0.247678472 / 928.61 | 0.247605160 / 881.39 | 1180.67 / 881.39 |

### 기준 대비 변화와 결과 해석

- EXP-072 strict는 EXP-051 대비 Skill이 2022 `+7.79`, 2023 `-2.15`, 2024 `+1.51`이었다. 평균과 최저는 소폭 높았지만 2023을 함께 개선하지 못했다.
- EXP-073 strict는 2022 `+5.79`, 2023 `-1.60`, 2024 `+1.15`였다. 타자 상태를 추가해도 투수 단독보다 평균과 최저가 낮았다.
- EXP-072 고정 `ar_k30_w050`은 2024 `883.01`까지 높였지만 2023은 `928.03`으로 내려갔다. 상태 보정 강도를 늘릴수록 시즌 간 교차 손실이 커졌다.
- 같은 2024 정답으로 보정 가중치를 최적화한 비배포 상한도 EXP-072 `Brier 0.247593606 / Skill 886.01`, EXP-073 `0.247593776 / 885.94`였다. 상태공간 계열 자체가 1000에 필요한 행별 잔차 해상도를 제공하지 못한다.
